...
import translate_cmu as translate
...
def main():
  ...
  one_hot = False
  FLAGS = Object()
  FLAGS.data_dir = "../data/cmu_mocap"
  FLAGS.architecture = "basic"
  FLAGS.seq_length_in = 50
  FLAGS.seq_length_out = 25
  FLAGS.num_layers = 1
  FLAGS.size = 128
  ...
  with tf.Session(config=tf.ConfigProto( device_count = {"GPU": 0})) as sess:
    ...
    print("{0: <16} | {1:4d} | {2:4d} | {3:4d} | {4:4d} | {5:4d}".format("milliseconds", 80, 160, 320, 400, 1000))
    for action in actions:
      # if action == "washwindow":
      #   continue
      print("{0: <16} | {1:.2f} | {2:.2f} | {3:.2f} | {4:.2f} | {5:.2f}".format( action,
            errs_constant_frame[action][1], errs_constant_frame[action][3],
            errs_constant_frame[action][7], errs_constant_frame[action][9], errs_constant_frame[action][24]))
      for i in [1,3,7,9,24]:
        mean_error[i] += errs_constant_frame[action][i]
    print("{0: <16} | {1:.2f} | {2:.2f} | {3:.2f} | {4:.2f} | {5:.2f}".format( "average",
            mean_error[1]/7, mean_error[3]/7,
            mean_error[7]/7, mean_error[9]/7, mean_error[24]/7))
